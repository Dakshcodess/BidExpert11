...
const Lobby = () => {
  ...
  const shareWhatsApp = () => {
    window.open(`https://wa.me/?text=Join my BidExpert11 auction! Room code: ${roomCode}`, '_blank')
  }
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        <div className="bg-[#141414] border border-white/10 rounded-2xl p-6 mb-6">
          <div className="flex justify-between items-start">
            <div>
              <p className="text-gray-500 text-xs uppercase tracking-wider mb-1">Auction Room</p>
              <h2 className="text-2xl font-bold text-white">{room?.name}</h2>
              <div className="flex items-center gap-4 mt-2">
                <span className="text-gray-500 text-sm">
                  Host: <span className="text-gray-300 font-medium">{room?.hostId?.name}</span>
                </span>
                <span className="text-gray-500 text-sm">
                  Purse: <span className="text-[#F59E0B] font-bold">₹{room?.pursePerTeam} Cr</span>
                </span>
                <span className="text-gray-500 text-sm">
                  Timer: <span className="text-white font-medium">{room?.bidTimer || 15}s</span>
                </span>
              </div>
            </div>
            <div className="text-right">
              <p className="text-gray-600 text-xs mb-1">Room Code</p>
              <p className="text-3xl font-bold tracking-widest text-[#F59E0B]">{roomCode}</p>
              <button
                onClick={copyCode}
        ...
        <div className="bg-[#141414] border border-white/10 rounded-2xl p-6 mb-6">
          ...
          <p className="text-gray-600 text-sm mb-4">Share the room code to invite others</p>
        ...
        {!isHost && (
          <div className="text-center py-4">
            ...
            <span className="text-gray-500 text-sm">Waiting for host to start the auction...</span>
          </div>
        )}
      </div>
    </div>
